...
Генератор JSON Fake BODY для запитів на REST API. Інструмент для полегшення тестування 
...
1. Додайте файл з json schem за шляхом `< file_name>.json` в `backend/schemas/`.
2. Оновіть сторінку фронту (папка змонтована як volume).

Поля `title` та `description` в середині сшеми це назви значеннь які буде застосовано в дробдауні на фронті

### Ключові слова в JSON Schema

- `type`: `object`, `array`, `string`, `integer`, `number`, `boolean`, `null`
- `properties`, `required` — для `object`
- `items`, `minItems`, `maxItems` — для `array`
- `format` (`email`, `date`, `date-time`, `uuid`, `uri`, `hostname`, `ipv4`, `ipv6`, `phone`)
- `enum` — рандомний вібур із списку значеннь
- `const` — фіксовані значення
